...
(function ($) {
    $.fn.jqueryUploadFile = function (userOptions) {
        ...
        function renderGrid(fileCollection, isDelete) {
            ...
            mainDiv.find(".maintable").find("tr.contentRow").length>0?mainDiv.find(".maintable").find("tr.contentRow").remove()://console.log("first");
        ...
        function addDocumentToFolder(folderPath){
            var call0 = checkIfFolderExists(folderPath);
            call0.done(function(data, textStatus, jqXHR) {
                ////console.log("call check folder done", data);
                for (var i = 0, f; f = obj.collFiles[i]; i++) 
                { 
                    var call = addDocToLib(f.fileArray, folderPath, f.fileName);    
                    call.done(function(data, textStatus, jqXHR) {
                        //console.log("call addDocToLib done", data); 
                        obj.checkCounter++;               
                        ...
                        var call2 = getItem(data.d);        
                            call2.done(function(data, textStatus, jqXHR) {
                                var item = data.d;                                  
                                //console.log("call2 done", item.Id);
                            });         
            ...
            call0.fail(function(jqXHR, textStatus, errorThrown) {
        ...
        function addDocumentToList(){
            ...
            for (var i = 0, f; f = obj.collFiles[i]; i++) 
            {                                   
                var call = addDocToList(f.fileArray, szId, f.fileName);    
                call.done(function(data, textStatus, jqXHR) {                    
                    obj.checkCounter++;               
        ...
        //async function to get the SPItem of teh SPFile
